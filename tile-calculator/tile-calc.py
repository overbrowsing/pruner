...
def print_metrics(original_breakpoints, best_tile_width, original_height, num_columns, num_rows, pixel_waste_results):
  print(f"\nTile Calculator Results")
  print(f"Breakpoints: {original_breakpoints} (px)")
  print(f"Columns = {num_columns}, Rows = {num_rows}")
  print(f"Tile Dimensions = {best_tile_width} x {original_height // num_rows}px")
  for bp in original_breakpoints:
    cols = (bp + best_tile_width - 1) // best_tile_width
    rows = original_height // (original_height // num_columns)
    print(f"--- Tiles Visible at {bp}px = {cols} (W) {rows} (H)")
  sorted_pixel_waste = dict(sorted(pixel_waste_results.items(), key=lambda item: item[1]))
  print("\nPixel Waste Results")
  print("Average pixel waste across viewports for the tested tile dimensions.")
  for tile_width, waste in sorted_pixel_waste.items():
    print(f"{tile_width} x {original_height // num_rows}px = {waste:.2f}px wasted")
...
def calculate_pixel_waste_for_breakpoints_with_increments(original_breakpoints, best_tile_width):
  pixel_waste_data = {}
  for i in range(len(original_breakpoints) - 1):
    start = original_breakpoints[i]
    end = original_breakpoints[i + 1]
    for viewport in range(start, end + 1):
      pixel_waste = calculate_metrics(viewport, best_tile_width)[1]
      pixel_waste_data[viewport] = pixel_waste
  return pixel_waste_data

def calculate_pixel_wastage_for_breakpoints(breakpoints, best_tile_width, original_height):
  pixel_waste_for_breakpoints = {}
  for bp in breakpoints:
    ...
    pixel_waste_for_breakpoints[bp] = width_waste + height_waste
  return pixel_waste_for_breakpoints
...
def main():
  ...
  pixel_waste_breakpoint_values = calculate_pixel_wastage_for_breakpoints(rounded_breakpoints, best_tile_width, original_height)
  
  num_columns = original_width // best_tile_width
  num_rows = original_height // (original_height // num_columns)
  
  print_metrics(original_breakpoints, best_tile_width, original_height, num_columns, num_rows, pixel_waste_results)
  
  export_csv = input("Do you want to export pixel waste data to CSV? (y/n): ").strip().lower()
  if export_csv == 'y':
    pixel_waste_for_csv = calculate_pixel_waste_for_breakpoints_with_increments(rounded_breakpoints, best_tile_width)
