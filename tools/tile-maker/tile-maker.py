...
def split_image(image, pruner_folder, base_name, grid_size, quality):
  ...
  for row in range(grid_size[1]):
    for col in range(grid_size[0]):
      ...
      tile.save(os.path.join(pruner_folder, tile_name), format=IMAGE_FORMAT, quality=quality, optimize=True)
...
def process_image(input_folder, target_image_name, img_size, grid_size, export_html_choice, quality):
  ...
  tile_count, total_size_kb, tile_dimensions = split_image(img, pruner_folder, os.path.splitext(target_image_name)[0], grid_size, quality)
...
def get_user_input():
  ...
  quality = 80
  if input("Do you want to set the compression quality? (y/n): ").lower() == 'y':
    quality = int(input("Enter the compression quality (0 to 80): "))
  ...
  return target_image_name, img_size, (columns, rows), target_choice, export_html_choice, quality
...
target_image_name, img_size, grid_size, target_choice, export_html_choice, quality = get_user_input()
total_tile_count = total_size_kb_all = total_image_count = 0

if target_choice == 'one':
  tile_count, total_size_kb, tile_dimensions = process_image(input_folder, target_image_name, img_size, grid_size, export_html_choice, quality)
  if tile_count > 0:
  ...
  for filename in os.listdir(input_folder):
    if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')):
      tile_count, total_size_kb, tile_dimensions = process_image(input_folder, filename, img_size, grid_size, export_html_choice, quality)
      if tile_count > 0:
